...
export default class Balance extends Component<Props, State> {
  ...
  switchCurrency = () => {
    ...
    if (displayCurrency === 'fiat') {
      this.setState({
        displayCurrency: 'NaCa'
      });
      configManager.modifyConfig('displayCurrency', 'NaCa');
      eventEmitter.emit('modifyCurrency', 'NaCa');
    }
  ...
  render() {
    ...
    if (displayCurrency === 'NaCa') {
      balanceTooltip =
        `Unlocked: ${atomicToHuman(unlockedBalance, true)} ${il8n.NACA}<br>` +
        `Locked: ${atomicToHuman(lockedBalance, true)} ${il8n.NACA}`;
